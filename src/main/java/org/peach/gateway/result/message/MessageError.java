...
/**
 * 与标准 HTTP 状态一一对应的网关侧文案与末四位数值，供 {@link org.peach.gateway.result.web.ErrorResult#httpStatus(MessageError, String)}
 * 等拼装 {@code code} 使用。
 * <p>
 * 枚举值覆盖常见 4xx/5xx；{@link #formStatus(int)} 对未在表中显式注册的状态码回落为
...
public enum MessageError {
	...
	/** 请求体过大（与 RFC 9110 Content Too Large / Payload Too Large 对应） */
	PAYLOAD_TOO_LARGE(HttpStatus.PAYLOAD_TOO_LARGE, 4022, "请求体过大"),
	/** URI 过长 */
	...
	/** 我是茶壶（RFC 2324） */
	I_AM_A_TEAPOT(HttpStatus.I_AM_A_TEAPOT, 4027, "我是茶壶（协议保留语义）"),
	/** 请求被错误定向 */
	MISDIRECTED_REQUEST(HttpStatus.MISDIRECTED_REQUEST, 4028, "请求被定向到无法处理的服务器"),
	/** 语义错误，无法处理 */
	UNPROCESSABLE_ENTITY(HttpStatus.UNPROCESSABLE_ENTITY, 4029, "语义错误，无法处理该请求"),
	/** 资源被锁定 */
	...
	/** 带宽限制 */
	BANDWIDTH_LIMIT_EXCEEDED(HttpStatus.BANDWIDTH_LIMIT_EXCEEDED, 5010, "已超出带宽或速率限制"),
	/** 需要扩展 */
	NOT_EXTENDED(HttpStatus.NOT_EXTENDED, 5011, "需要获取更多扩展信息才能完成请求"),
	/** 需要网络身份验证 */
